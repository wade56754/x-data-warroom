...
Cron 配置（launchd / cron）：
  */15 * * * * python3 tweet_growth_cli.py --run --fast  >> ~/.x-data/growth.log 2>&1
  0 * * * *    python3 tweet_growth_cli.py --run --normal >> ~/.x-data/growth.log 2>&1

环境变量：
  X_DATA_DIR        — 数据目录（默认 ~/.x-data/）
  TWEET_GROWTH_DATA — 数据文件路径（默认 $X_DATA_DIR/data.json）
"""
...
def _do_run(fast_mode: bool):
    ...
    # 每次 CLI run 开一个 DB 连接，采样循环内复用；DB 失败不阻塞主流程
    try:
        import db as _db
    ...
    # 48h 窗口过滤（仅 fast 模式）：解析 latest.created_at 或顶层 created_at
    def _get_tweet_created_at(rec: dict) -> "datetime | None":
        ca_str = (rec.get("latest") or {}).get("created_at") or rec.get("created_at")
        if not ca_str:
            return None
        try:
    ...
    fast_cutoff = now_utc - timedelta(hours=FAST_MODE_MAX_AGE_HOURS)

    sampled = 0
    skipped_age = 0
    for tweet_id, record in tweets.items():
        # fast 模式：跳过 created_at 未知或超过 48h 的推
        if fast_mode:
            ca = _get_tweet_created_at(record)
            if ca is None or ca < fast_cutoff:
                skipped_age += 1
                continue
        ...
        # DB double-write: insert sample point
        if _db_conn is not None:
            snap = (updated.get("history") or [{}])[-1]
            if snap:
                try:
                    _db.insert_sample(_db_conn, tweet_id, snap)
                except Exception as e:
                    print(f"[WARN] DB insert_sample({tweet_id}) 失败: {e}", file=sys.stderr)
    ...
    tg.save_data(data)
    if fast_mode:
        eligible = len(tweets) - skipped_age
        print(f"[{ts}] 完成，采样 {sampled}/{eligible} 条（48h 内）", flush=True)
        print(f"[INFO] 采样 {sampled}/{len(tweets)} 条（48h 内）", flush=True)
    else:
